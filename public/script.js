...
document.getElementById("redirectLink").addEventListener("click", function() {
  ...
  userRef.get().then(function(doc) {
    if (doc.exists) {
      var cooldown = new Date(doc.data().shrinkcool).getTime();
      var currentTime = new Date().getTime();
      var timeDifference = currentTime - cooldown;
      var cooldownHours = timeDifference / (1000 * 60 * 60); // Convert milliseconds to hours

      if (cooldownHours >= 24) {
        window.location.href = "https://shrinke.me/faucetweb";
      } else {
        var remainingCooldownHours = 24 - cooldownHours;
        alert("Please wait for " + remainingCooldownHours + " more hours doing the shortlink again!");
      }
